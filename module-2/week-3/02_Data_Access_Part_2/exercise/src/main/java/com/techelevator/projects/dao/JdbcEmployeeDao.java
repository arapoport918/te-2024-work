...
public class JdbcEmployeeDao implements EmployeeDao {
	...
	public Employee updateEmployee(Employee employee) {
		...
		String sql = "UPDATE employee SET department_id = ?, first_name = ?, last_name = ?, birth_date = ?, hire_date = ? " +
				"WHERE employee_id = ?;";
		try {
			int numOfRows = jdbcTemplate.update(sql, employee.getDepartmentId(), employee.getFirstName(), employee.getLastName(),
	...
	public int deleteEmployeeById(int id) {
		String deleteFromProjEmp = "DELETE FROM project_employee WHERE employee_id = ?;";
		String deleteFromEmp = "DELETE FROM employee WHERE employee_id = ?;";
		int rows = 0;

		try {
			jdbcTemplate.update(deleteFromProjEmp, id);
			rows = jdbcTemplate.update(deleteFromEmp, id);
		} catch (CannotGetJdbcConnectionException e) {
			throw new DaoException("Unable to connect to server or database", e);
		} catch (DataIntegrityViolationException e) {
			throw new DaoException("Data integrity violation", e);
...
//		String deleteEmployeeFromProjEmp = "DELETE FROM project_employee WHERE employee_id = ?;";
//		String getEmployee_id = "SELECT employee_id FROM employee WHERE department_id = ?;";
//		String deleteEmployeeByDept = "DELETE FROM employee WHERE department_id = ?;";

		int rows = 0;

		try {
			jdbcTemplate.update(deleteFromDepartment, departmentId);
			rows = jdbcTemplate.update(deleteFromEmployee, departmentId);
		} catch (CannotGetJdbcConnectionException e) {
			throw new DaoException("Unable to connect to server or database", e);
		} catch (DataIntegrityViolationException e) {
			throw new DaoException("Data integrity violation", e);
