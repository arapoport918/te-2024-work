...
public class Exercise02_CanDrive {
    ...
    public boolean canDrive(boolean hasPermit, boolean withLicensedPassenger, int passengerAge) {
        final int AGE_MINIMUM = 21;
        boolean isAbleToDrive;

        if (hasPermit && withLicensedPassenger && passengerAge >= AGE_MINIMUM) {
            isAbleToDrive =true;
        } else {
            isAbleToDrive = false;
        }
    ...
    public boolean canDrive(boolean hasPermit, boolean withLicensedPassenger, int passengerAge, boolean isPassengerOurGuardian) {
        boolean isAbleToDrive;
        final int AGE_MIN = 21;
        final int AGE_MIN_GUARDIAN = 18;

        if (hasPermit && withLicensedPassenger && isPassengerOurGuardian && passengerAge >= AGE_MIN_GUARDIAN) {
            isAbleToDrive =true;
        } else if (hasPermit && withLicensedPassenger && passengerAge >= AGE_MIN) {
            isAbleToDrive = true;
        } else {
            isAbleToDrive = false;
        }
