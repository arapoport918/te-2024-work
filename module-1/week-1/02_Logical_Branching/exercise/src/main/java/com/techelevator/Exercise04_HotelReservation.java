...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        ...
        if (numberOfNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
            totalCost = DAILY_RATE * numberOfNights;
        } else {
            totalCost = DISCOUNT_RATE * numberOfNights;
        }
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking) {
        ...
        if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE && includesParking) {
            totalCost = (DAILY_RATE * numOfTotalNights) + (PARKING_RATE * numOfTotalNights);
        } else if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE && !includesParking) {
            totalCost = DAILY_RATE * numOfTotalNights;
        } else if (numOfTotalNights >= MINIMUM_NIGHTS_FOR_DISCOUNT_RATE && includesParking) {
            totalCost = (DISCOUNT_RATE * numOfTotalNights) + (PARKING_RATE * numOfTotalNights);
        } else {
            totalCost =DISCOUNT_RATE * numOfTotalNights;
        }
