...
public class Exercise06_RaceDay {
    ...
    public int determineRaceBlock(int age, boolean isEarlyRegistration) {
        int raceBlock;
        final int AGE_CUTOFF = 18;

        if (age >= AGE_CUTOFF && isEarlyRegistration) {
            raceBlock = 1;
        } else if (age >= AGE_CUTOFF && !isEarlyRegistration) {
            raceBlock = 2;
        } else {
            raceBlock = 3;
        }
    ...
    public int getBibNumber(int age, int registrationNumber, boolean isEarlyRegistration) {
        int bibNumber = registrationNumber;
        final int AGE_CUTOFF = 18;
        final int BIB = 1000;

        if (age >= AGE_CUTOFF && isEarlyRegistration) {
            bibNumber =registrationNumber + BIB;
        }
    ...
    public int getConfirmedBibNumber(int age, int registrationNumber, boolean isEarlyRegistration) {
        ...
        if (age >= AGE_CUTOFF && isEarlyRegistration) {
            bibNumber =registrationNumber + BIB;
        } else if (registrationNumber > BIB && !isEarlyRegistration) {
            bibNumber = -1;
        }
