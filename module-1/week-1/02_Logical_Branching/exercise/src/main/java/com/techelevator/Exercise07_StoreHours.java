...
public class Exercise07_StoreHours {
    ...
    public boolean isStoreOpen(int currentHour) {
        final int OPEN_TIME = 8;
        final int CLOSE_TIME = 17;

        if (currentHour >= OPEN_TIME && currentHour < CLOSE_TIME) {
            return true;
        }
    ...
    public boolean isStoreOpen(int currentHour, char currentDay) {
        ...
        if (currentHour >= OPEN_TIME && currentHour < CLOSE_TIME && (currentDay == 'M' || currentDay == 'W' || currentDay =='F')) {
            return true;
        }
    ...
    public boolean isStoreOpen(int currentHour, char currentDay, boolean isSummer) {
        final int OPEN_TIME = 8;
        final int CLOSE_TIME = 17;
        final int CLOSE_TIME_W_SUMMER = 20;
        final int OPEN_TIME_SAT = 9;
        final int CLOSE_TIME_SAT = 15;

        if (isSummer) {
            if (currentDay == 'W') {
                if (currentHour >= OPEN_TIME && currentHour < CLOSE_TIME_W_SUMMER) {
                    return true;
                }
            } else if (currentDay == 'S') {
                if (currentHour >= OPEN_TIME_SAT && currentHour < CLOSE_TIME_SAT) {
                    return true;
                }
