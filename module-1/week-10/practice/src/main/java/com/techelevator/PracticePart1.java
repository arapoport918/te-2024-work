...
public class PracticePart1 {
    ...
    */
    public boolean Q02_isWaterTemperatureExtreme(int temperature) {
        final int BELOW_FREEZING = 32;
        final int ABOVE_BOILING = 212;

        if (temperature <= BELOW_FREEZING || temperature >= ABOVE_BOILING) {
            return true;
        }
    ...
    */
    public boolean Q03_isLeapYear(int year) {
        ...
        final int LEAP_YEAR_400 = 400;
        final int LEAP_YEAR_100 = 100;
        final int LEAP_YEAR_4 = 4;

        return year % LEAP_YEAR_400 == 0 || (year % LEAP_YEAR_100 != 0 && year % LEAP_YEAR_4 == 0);
    }
    ...
    */
    public int Q05_sumOfNumbersDivisibleBy7Between(int lowestNumber, int highestNumber) {
        final int DIVISIBLE_BY_7 = 7;
        int sum = 0;

        for (int i = lowestNumber; i <= highestNumber; i++) {
            if (i % DIVISIBLE_BY_7 == 0) {
                sum += i;
            }
        }
    ...
    */
    public String Q06_stringBetweenTags(String stringToSearch, String tag) {
        ...
        int startingPosition = stringToSearch.indexOf(tag) + tag.length();
        int endingPosition = stringToSearch.lastIndexOf(tag);

        //indexOf can return -1 so you need to have checks in place to account for this measure
        //also make sure that startingPosition has a lower index than the ending
        if(startingPosition >= 0 && endingPosition >= 0 && startingPosition <= endingPosition) {
            return stringToSearch.substring(startingPosition, endingPosition);
        }
...
//            return "";
//        } else if (stringToSearch.equals(tag + tag)) {
//            return "";
//        }
//
//        String firstTagSubString = stringToSearch.substring(stringToSearch.indexOf(tag) + tag.length());
//        int secondTagLocation = firstTagSubString.indexOf(tag);
//
//        if (!firstTagSubString.contains(tag)) {
//            return "";
//        }
//
//        betweenTags = firstTagSubString.substring(0, secondTagLocation);
    ...
    */
    public int Q08_allowanceCalculator(int weekly, int gift, int bonus) {
        final int QUARTERS_IN_A_YEAR = 4;
        final int WEEKS_IN_A_YEAR = 52;

        int allowance = weekly + (gift + bonus * QUARTERS_IN_A_YEAR) / WEEKS_IN_A_YEAR;
    ...
    */
    public boolean Q09_isIncreasing(int[] numbers) {
        int numberTracker = numbers[0];

        for (int i = 1; i < numbers.length; i++) {
            if (numbers[i] > numberTracker) {
                numberTracker = numbers[i];
            }
        ...
        if (numbers[numbers.length - 1] == numberTracker) {
            return true;
        }
    ...
    */
    public List<Integer> Q10_reverseList(List<Integer> numbers) {
        ...
        //Collections.reverse(numbers); also works
    ...
    */
    public Map<String, Integer> Q11_idealPets(String[] suggestedPets) {
        Map<String, Integer> petList = new HashMap<>();
        final int STARTING_VALUE = 1;

        for (String pet : suggestedPets) {
            if (petList.containsKey(pet)) {
                int value = petList.get(pet);
                value++;
                petList.put(pet, value);

            } else {
                petList.put(pet, STARTING_VALUE);
            }
        }
